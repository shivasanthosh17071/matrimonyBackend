...
const activatePlan = async (userId, plan, billingCycle, paymentId) => {
  ...
  await User.findByIdAndUpdate(userId, { plan, planActivatedAt: new Date(), planExpiresAt: expiresAt });
  const p = await Payment.findByIdAndUpdate(paymentId, { status: 'captured', planStartDate: new Date(), planEndDate: expiresAt }, { new: true });
  const user = await User.findById(userId);
  if (user?.email) sendPlanConfirmationEmail(user, plan, expiresAt).catch(e => logger.error(`Plan email: ${e.message}`));
  return expiresAt;
};
...
exports.createRazorpayOrder = catchAsync(async (req, res, next) => {
  ...
  if (!PRICES[plan]) return next(new AppError('Invalid plan.', 400, 'INVALID_PLAN'));
  ...
  const payment = await Payment.create({
    user: req.user._id, plan, gateway: 'razorpay',
    currency: 'INR', amountPaise: amount, billingCycle,
    status: 'created', razorpayOrderId: order.id,
  });

  return sendSuccess(res, {
    orderId: order.id, amount, currency: 'INR',
    key: process.env.RAZORPAY_KEY_ID, paymentId: payment._id,
...
exports.verifyRazorpayPayment = catchAsync(async (req, res, next) => {
  ...
  if (expected !== razorpay_signature)
    return next(new AppError('Payment verification failed. Contact support if amount was deducted.', 400, 'SIGNATURE_MISMATCH'));
  ...
  const expiresAt = await activatePlan(req.user._id, payment.plan, payment.billingCycle, payment._id);
  return sendSuccess(res, { plan: payment.plan, planExpiresAt: expiresAt }, `${payment.plan} plan activated!`, 200);
});
...
exports.createStripeSession = catchAsync(async (req, res, next) => {
  ...
  if (!PRICES[plan]) return next(new AppError('Invalid plan.', 400, 'INVALID_PLAN'));
  if (!PRICES[plan][currency]) return next(new AppError('Currency not supported.', 400, 'INVALID_CURRENCY'));
  ...
  const session = await stripe.checkout.sessions.create({
    ...
    line_items: [{
      price_data: {
        ...
        product_data: {
          name: `Telugu Rishtey — ${plan.charAt(0).toUpperCase() + plan.slice(1)} Plan`,
  ...
  await Payment.create({
    user: req.user._id, plan, gateway: 'stripe',
    currency, amountPaise: amount, billingCycle,
    status: 'pending', stripeSessionId: session.id,
  });

  return sendSuccess(res, { sessionId: session.id, url: session.url }, 'Stripe session created.', 201);
});
  ...
  if (event === 'payment.captured') {
    try {
      ...
      if (payment && payment.status !== 'captured') {
        payment.webhookReceivedAt = new Date(); payment.webhookEvent = event;
        await payment.save();
        await activatePlan(payment.user, payment.plan, payment.billingCycle, payment._id);
      }
    } catch (e) { logger.error(`Razorpay webhook error: ${e.message}`); }
  }
  if (event === 'payment.failed') {
    const entity = payload?.payment?.entity;
    await Payment.findOneAndUpdate({ razorpayOrderId: entity?.order_id }, { status: 'failed', failureReason: entity?.error_description, webhookReceivedAt: new Date() });
  }
  ...
  if (event.type === 'checkout.session.completed') {
    ...
    try {
      ...
      if (payment && payment.status !== 'captured') {
        ...
        payment.webhookReceivedAt = new Date(); payment.webhookEvent = event.type;
        await payment.save();
        await activatePlan(userId, plan, billingCycle, payment._id);
      }
    } catch (e) { logger.error(`Stripe webhook error: ${e.message}`); }
  }
  if (event.type === 'payment_intent.payment_failed') {
    await Payment.findOneAndUpdate({ stripePaymentIntentId: event.data.object.id }, { status: 'failed', failureReason: event.data.object.last_payment_error?.message, webhookReceivedAt: new Date() });
  }
  res.json({ received: true });
};
